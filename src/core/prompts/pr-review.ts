...
export function buildPRSummaryPrompt(
  findings: import('../../types/index').Finding[],
  reviewerErrors?: { id: string; name: string; message: string }[]
): string {
  const bySeverity = {
  ...
  const errorSection = reviewerErrors && reviewerErrors.length > 0
    ? `\n> ⚠️ **${reviewerErrors.length} reviewer(s) failed and were skipped:**\n${reviewerErrors.map(e => `> - \`${e.name}\`: ${e.message}`).join('\n')}\n`
    : '';
