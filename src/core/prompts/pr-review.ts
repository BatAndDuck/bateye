...
export function buildOrchestratorSystemPrompt(availableReviewers: { id: string; name: string; description?: string; scopeHints?: string[] }[]): string {
  ...
  return `You are a PR review orchestrator. Given a pull request diff and changed files, select only the reviewers that are clearly relevant to investigating this PR.
...
- Be relevant, not exhaustive. Prefer fewer reviewers when the change is narrow and localized.
- Scope by file type and subsystem. Include only reviewers that match the changed files, surrounding subsystem, or attached tool coverage.
- Prefer 1-2 reviewers for small, localized changes unless the diff clearly spans multiple domains.
- Include tool-enhanced scanners only when the changed files match the scanner domain and the tool can materially validate the change.
- Avoid selecting broad reviewers when the diff is clearly unrelated to their specialty.
...
${verificationSection}${scannerSection}
---
