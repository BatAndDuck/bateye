...
export type PRReviewResult = {
  ...
  headRef: string;
  status: ReviewRunStatus;
  selectedReviewers: ReviewerPlanSelection[];
  summary: string;
  findings: PRFinding[];
  issues: ReviewIssue[];
...
export type ReviewerPlanSelection = {
  reviewerId: string;
  reason: string;
  /** 0–1 confidence that this reviewer is relevant. Used for trimming when maxReviewers is set. */
  confidence: number;
  /** Compact reviewer-facing briefing generated by the deep planner. */
  briefing?: string;
  /** File or folder paths the reviewer should inspect first. */
  contextPaths?: string[];
  /** End-to-end flows the reviewer should trace while investigating. */
  verticalFlows?: string[];
  /** Repo-derived business intent or domain context relevant to the change. */
  businessContext?: string[];
  /** Similar implementations to compare against for consistency. */
  consistencyReferences?: string[];
  /** Existing nearby tests or test gaps that matter for this change. */
  testLocations?: string[];
  /** Short planner-discovered issue hints worth checking before broader exploration. */
  issueHints?: string[];
};
